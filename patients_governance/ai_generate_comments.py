...
target_schema = "patients_governance.silver"
tables = spark.sql(f"SHOW TABLES IN {target_schema}").collect()

for table in tables:
    t_name = table.tableName
    columns = spark.sql(f"DESCRIBE {target_schema}.`{t_name}`").collect()
    ...
    for col in columns:
        ...
        try:
            spark.sql(f"ALTER TABLE {target_schema}.`{t_name}` ALTER COLUMN `{col_name}` COMMENT '{description}'")
...
print("\nLLM Metadata is live in Unity Catalog.")
